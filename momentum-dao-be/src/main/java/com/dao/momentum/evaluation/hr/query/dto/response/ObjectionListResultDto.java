...
public class ObjectionListResultDto {
    ...
    @Schema(description = "가중치: 조직 기여 및 협업 행동 (%)", example = "15")
    private Integer weightTeam;

    @Schema(description = "가중치: 자기관리 및 태도 (%)", example = "15")
    private Integer weightAttitude;

    @Schema(description = "가중치: 업무 태도 및 몰입도 (%)", example = "25")
    private Integer weightImmersion;

    @Schema(description = "가중치: 정량 성과 및 목표관리 (%)", example = "20")
    private Integer weightResult;

    @Schema(description = "가중치: 보정 점수 (%)", example = "5")
    private Integer weightAdjust;
